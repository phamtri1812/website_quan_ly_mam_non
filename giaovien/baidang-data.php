<?php
    ...
    if(isset($_POST["theloai"])){
        ...
        while($row=$q->fetch_assoc()){
            echo "<tr>
                    ...
                    <td style='width: 400;'>".$row["TIEUDE"]."</td>
                    <td id='cot-an'>".$row["LINK"]."</td>
                    <td style='width: 100;'>".$row["THELOAI"]."</td>
                    <td style='width: 100;'>".$row["NGAYDANG"]."</td>
                    <td style='width: 200;'><button id='btn-xem' onclick='xem(this)'><i class='bi bi-eye'></i></button>
    ...
    if(isset($_POST["chudexoa"])){
        $x=$conn->query("SELECT LINK FROM `anh` WHERE CHUDE='".$_POST["chudexoa"]."'");
        while($r=$x->fetch_assoc()){
            unlink($r["LINK"]);
        }
