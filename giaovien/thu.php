<?php
    ...
    ##########################################################
        #Xuất file excel
            ...
            $s=$conn->query("SELECT count(*) FROM `hocsinh` WHERE MSLOP='".$_SESSION["lop"]."'");
            while($r1=$s->fetch_assoc()){
                $c=$r1["count(*)"];
            }

            $s1=$conn->query("SELECT * FROM lop WHERE MSLOP='".$_SESSION["lop"]."'");
            while($r1=$s1->fetch_assoc()){
                $lop=$r1["TENLOP"];
            }

            $data=array();
            $q=$conn->query("SELECT * FROM `hocsinh` WHERE MSLOP='".$_SESSION["lop"]."'");
